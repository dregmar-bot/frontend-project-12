...
const ChannelBox = () => {
  ...
  const handleOpenModal = (type, e) => {
    dispatch(openModal({ type, channel: Number(e.target.id)}))
  };
    ...
    return (
      <Nav.Item as="li" className="w-100" key={channel.id} id={channel.id}>
        <Dropdown as={ButtonGroup} className="d-flex" key={channel.id}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item id={channel.id} href="#" onClick={(e) => handleOpenModal('remove', e)}>{t('chatPage.channelBox.remove')}</Dropdown.Item>
            <Dropdown.Item id={channel.id} href="#" onClick={(e) => handleOpenModal('rename', e)}>{t('chatPage.channelBox.rename')}</Dropdown.Item>
          </Dropdown.Menu>
        </Dropdown>
      </Nav.Item>
    );
  ...
  return (
    <div className="col-4 col-md-2 border-end px-0 bg-light flex-column h-100 d-flex">
      <div className="d-flex mt-1 justify-content-between mb-2 ps-4 pe-2 p-4">
        ...
        <Button className="p-0 text-primary btn-group-vertical" variant="link" onClick={(e) => handleOpenModal('add', e)}>
